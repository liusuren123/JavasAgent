...
### Step 1：浏览器搜索场景 — 基础链路验证
- [ ] T1.1：编写端到端测试脚本 `tests/e2e/test_browser_search.py`
  - 打开浏览器 → 输入关键词 → 搜索 → 打开第一个结果 → 截图验证
- [ ] T1.2：运行测试脚本，记录每个环节的成功/失败状态
- [ ] T1.3：将失败原因分类（感知失败/定位失败/操作失败/决策失败）
- [ ] T1.4：输出测试报告到 `test-evidence/e2e-browser-search/`

### Step 2：记事本文本编辑场景
- [ ] T2.1：编写端到端测试脚本 `tests/e2e/test_notepad_edit.py`
  - 打开记事本 → 输入中文文本 → 保存到指定路径 → 验证文件内容
- [ ] T2.2：运行并记录结果
- [ ] T2.3：输出测试报告到 `test-evidence/e2e-notepad/`

### Step 3：Excel 表格操作场景
- [ ] T3.1：编写端到端测试脚本 `tests/e2e/test_excel_basic.py`
  - 打开 Excel → 创建表格 → 输入表头和数据 → 保存
- [ ] T3.2：运行并记录结果
- [ ] T3.3：输出测试报告到 `test-evidence/e2e-excel/`

### Step 4：验证总结 + 问题清单
- [ ] T4.1：汇总 3 个场景的测试结果，生成验证报告
- [ ] T4.2：整理问题清单，按严重程度排序
- [ ] T4.3：确认 Phase 2（UI 检测）的具体需求是否需要调整
- [ ] T4.4：验证报告同步给产品经理
